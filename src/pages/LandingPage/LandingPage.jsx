...
export default function LandingPage() {
  ...
  useEffect(() => {
    async function loadUser() {
      ...
      if (!token) {
        ...
        try {
          ...
          // Fallback to token display name if API fails
    ...
    async function loadCategories() {
      try {
        const res = await api.get("/api/category?limit=100");
    ...
    async function loadInitialData() {
      ...
      try {
        ...
        const results = await Promise.allSettled([
          ProductService.getNewProduct(4),
          ProductService.getBestSellerProduct(4),
          api.get("/api/store/top-store?limit=5"),
          api.get("/api/voucher/top-voucher?limit=4"),
        ]);
        ...
        // Sử dụng dữ liệu đã được ProductService mapping chuẩn
        const mappedNew =
          newProductsDB.length > 0 ? newProductsDB : mockProducts;
        const mappedBest =
          bestProductsDB.length > 0 ? bestProductsDB : mockPersonalized;
  ...
  useEffect(() => {
    ...
    async function loadCategoryProducts() {
      ...
      try {
        ...
        // Dữ liệu đã được mapping chuẩn trong ProductService
