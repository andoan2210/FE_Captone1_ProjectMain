...
const TheThongKe = ({ isActive: propIsActive }) => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true);
        const data = await CuahangService.getStoreStats();
        // Ưu tiên dùng propIsActive nếu có, nếu không thì dùng từ API
        const currentActive = propIsActive !== undefined ? propIsActive : data.activeStatus;

        setStats([
          { name: 'Tổng sản phẩm', value: data.totalProducts, icon: FiBox, color: 'text-blue-600', bg: 'bg-gradient-to-br from-blue-50 to-blue-100', iconColor: 'text-blue-500' },
          { name: 'Tổng đơn hàng', value: data.totalOrders, icon: FiShoppingCart, color: 'text-indigo-600', bg: 'bg-gradient-to-br from-indigo-50 to-indigo-100', iconColor: 'text-indigo-500' },
          { name: 'Tổng voucher', value: data.totalVouchers, icon: FiTag, color: 'text-violet-600', bg: 'bg-gradient-to-br from-violet-50 to-violet-100', iconColor: 'text-violet-500' },
    ...
    fetchStats();
  }, [propIsActive]); // Re-run whenever the parent status changes
